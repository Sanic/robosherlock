...
void RSProcessManager::runOnce()
{
  if (!secAeEmpty)
  {
    ...
    rs::CASConsumerContext::getInstance().addCAS(engineS_->getAAEName(), engineS_->getCas());
...
bool RSProcessManager::handleQuery(std::string &request, std::vector<std::string> &result)
{
  ...
  if(!new_pipeline_orders.empty())
  {
    ...
    if(queryType == QueryInterface::QueryType::DETECT)
    {
      ...
      engine_->processOnce();
      rs::CASConsumerContext::getInstance().addCAS(engine_->getAAEName(), engine_->getCas());
      engine_->resetPipelineOrdering();
